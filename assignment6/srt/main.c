...
//creating global variable for scene
scene_t scene;

//making global variable to keep track of number of threads
int nthreads;

//making constant colors to be used in the image
#define NUM_COLORS 3

//an array to hold all the values which need to be ultimately outputted
//this is required so that synchronization is not an issue while multithreading
float finalResult[width][height][NUM_COLORS];

//we need to create a function which we can thread
void * splitIt(void* threadNum)
{
    Vec3 camera_pos;
    ...
    for( int px = *(int*)threadNum; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            //now instead of outputting, we store the values in the result
            finalResult[px][py][0] = scaled_color[0];
            finalResult[px][py][1] = scaled_color[1];
            finalResult[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    //now we implement multithreading
    
    pthread_t threads[nthreads];
    int thread_id[nthreads]; //this keeps track of which thread is which
    
    //creating the threads...
    for(int y = 0; y < nthreads; y++)
    {
        thread_id[y] = y;
        int sts = pthread_create(&threads[y], NULL, splitIt, &thread_id[y]);
        if(sts)
        {
    ...
    //now we need to free the threads using pthread_join
    for(int l = 0; l < nthreads; l++)
    {
        int sts = pthread_join(threads[l], NULL);
        if(sts)
        {
            fprintf(stderr, "error in freeing threads");
            exit(-1);
        }
    }
    
    //all work is done now. time to output the result!
    ...
    for(int i = 0; i < width; i++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n",
            finalResult[i][j][0], finalResult[i][j][1], finalResult[i][j][2]);
        }
        printf("\n");
    }
